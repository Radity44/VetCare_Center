...
@section('content')
    ...
    <div class="max-w-2xl mx-auto bg-white p-6 rounded-xl shadow-lg">
        ...
        <form method="POST" action="{{ route('kunjungan.store') }}" class="space-y-4">
            ...
            <h3 class="text-lg font-bold text-gray-800 mb-2">Informasi Medis</h3>
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <select name="status_visit" id="status_visit" required
                        class="w-full rounded-lg border-2 border-gray-300 py-2.5 px-3 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white">
                        <option value="Scheduled" {{ old('status_visit') == 'Scheduled' ? 'selected' : '' }}>Scheduled
                            (Dijadwalkan)</option>
                        <option value="In Progress" {{ old('status_visit') == 'In Progress' ? 'selected' : '' }}>In
                            Progress (Sedang Berlangsung)</option>
                        <option value="Completed" {{ old('status_visit') == 'Completed' ? 'selected' : '' }}>Completed
                            (Selesai)</option>
                        <option value="Cancelled" {{ old('status_visit') == 'Cancelled' ? 'selected' : '' }}>Cancelled
                            (Dibatalkan)</option>
                    </select>
                    @error('status_visit')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
            ...
            <div class="flex justify-between mt-6">
                <button type="submit"
                    class="px-6 py-2.5 bg-blue-600 text-white rounded-lg hover:bg-blue-700 font-semibold transition shadow">
                    Simpan Kunjungan
                </button>
