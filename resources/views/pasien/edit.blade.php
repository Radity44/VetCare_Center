...
@section('content')
    <div class="max-w-2xl mx-auto bg-white p-6 rounded-xl shadow-lg">
        <h2 class="font-bold text-xl mb-4 text-blue-700">Edit Pasien: <span
                class="text-gray-800">{{ $pasien->nama_hewan }}</span></h2>
        ...
        <form method="POST" action="{{ route('pasien.update', $pasien) }}" class="space-y-4">
            ...
            <div>
                ...
                <div class="flex gap-2 items-center">
                    <input type="number" name="umur_hewan" min="0"
                ...
                <div>
                    ...
                    <select name="status" required
                        ...
                        @foreach (\App\Models\Patient::getStatusList() as $s)
                            <option value="{{ $s }}"
                                {{ old('status', $pasien->status ?? 'Booking') == $s ? 'selected' : '' }}>
                                {{ $s }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div>
                ...
                <input name="judul_perawatan" value="{{ old('judul_perawatan', $pasien->judul_perawatan) }}"
                        ...
                        @foreach ($pasien->visits as $visit)
                            <tr class="hover:bg-gray-50 transition">
                                ...
                                <td class="px-4 py-3 text-sm">
                                    ...
                                    @endphp
                                    <span
                                        class="px-2 py-1 rounded-full text-xs font-medium {{ $statusColors[$visit->status] ?? 'bg-gray-100 text-gray-800' }}">
                                        {{ $visit->status }}
                                    </span>
                                </td>
