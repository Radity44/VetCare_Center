        ...
        <table class="min-w-full">
            <thead>
                <tr class="bg-gradient-to-r from-blue-500 to-purple-600 text-white">
                    ...
                    <th class="py-4 px-4 text-left font-semibold text-sm">Nama Layanan</th>
                    <th class="py-4 px-4 text-left font-semibold text-sm">Deskripsi</th>
                    <th class="py-4 px-4 text-right font-semibold text-sm">Harga</th>
                    <th class="py-4 px-4 text-left font-semibold text-sm">Kategori</th>
                    <th class="py-4 px-4 text-center font-semibold text-sm">Durasi</th>
            ...
            <tbody class="text-sm">
                ...
                <tr class="border-b border-gray-100 hover:bg-blue-50 transition">
                    ...
                    <td class="py-4 px-4">
                        <span class="font-semibold text-gray-800">{{ $srv->nama_layanan }}</span>
                    </td>
                    <td class="py-4 px-4 text-gray-600">{{ Str::limit($srv->deskripsi, 50) }}</td>
                    <td class="py-4 px-4 text-right">
                        <span class="font-semibold text-blue-600">Rp {{ number_format($srv->harga, 0, ',', '.') }}</span>
                    </td>
                    <td class="py-4 px-4 text-gray-700">{{ $srv->kategori }}</td>
                    <td class="py-4 px-4 text-center text-gray-700">{{ $srv->durasi }} menit</td>
                    ...
                    <td class="py-4 px-4">
                        <div class="flex items-center justify-center gap-2">
                            ...
                            <form action="{{ route('layanan.destroy', $srv) }}" method="POST" 
                                  onsubmit="return confirm('Yakin hapus layanan {{ $srv->nama_layanan }}?')" 
                                  class="inline">
                                @csrf @method('DELETE')
                                <button type="submit" 
